gnomAD v4 - RAD51C - Population specific variant table & Comparison of unique and shared variants within population
Packages used
```{r}
library(tidyverse)
library(here)
...
RAD51Cv4 %>% 
  ...
  count(chromosome) -> Total_rad51c_afr
RAD51Cv4 %>% 
  filter(allele_count_african_african_american!=0, allele_count_remaining==0, allele_count_admixed_american==0, allele_count_ashkenazi_jewish==0, allele_count_east_asian==0, allele_count_european_finnish==0, allele_count_european_non_finnish==0 , allele_count_south_asian==0, allele_count_middle_eastern==0, allele_count_amish==0) %>% 
  count(chromosome) %>% 
  inner_join(Total_rad51c_afr, by="chromosome") %>% 
  mutate(n.z=n.y-n.x) %>% 
  rename("total" = "n.y", "unique" = "n.x", "shared" = "n.z") %>% 
  mutate(Population="AFR") %>% 
  select(Population, total, shared, unique) -> Unique_rad51c_afr

RAD51Cv4 %>% 
  filter(allele_count_admixed_american!=0) %>% 
  count(chromosome) -> Total_rad51c_amr
RAD51Cv4 %>% 
  filter(allele_count_african_african_american==0, allele_count_remaining==0, allele_count_admixed_american!=0, allele_count_ashkenazi_jewish==0, allele_count_east_asian==0, allele_count_european_finnish==0, allele_count_european_non_finnish==0 , allele_count_south_asian==0, allele_count_middle_eastern==0, allele_count_amish==0) %>% 
  count(chromosome) %>% 
  inner_join(Total_rad51c_amr, by="chromosome") %>% 
  mutate(n.z=n.y-n.x) %>% 
  rename("total" = "n.y", "unique" = "n.x", "shared" = "n.z") %>% 
  mutate(Population="AMR") %>% 
  select(Population, total, shared, unique) -> Unique_rad51c_amr

RAD51Cv4 %>% 
  filter(allele_count_european_non_finnish!=0) %>% 
  count(chromosome) -> Total_rad51c_nfe
RAD51Cv4 %>% 
  filter(allele_count_african_african_american==0, allele_count_remaining==0, allele_count_admixed_american==0, allele_count_ashkenazi_jewish==0, allele_count_east_asian==0, allele_count_european_finnish==0, allele_count_european_non_finnish!=0 , allele_count_south_asian==0, allele_count_middle_eastern==0, allele_count_amish==0) %>% 
  count(chromosome) %>% 
  inner_join(Total_rad51c_nfe, by="chromosome") %>% 
  mutate(n.z=n.y-n.x) %>% 
  rename("total" = "n.y", "unique" = "n.x", "shared" = "n.z") %>% 
  mutate(Population="NFE") %>% 
  select(Population, total, shared, unique) -> Unique_rad51c_nfe

RAD51Cv4 %>% 
  filter(allele_count_east_asian!=0) %>% 
  count(chromosome) -> Total_rad51c_eas
RAD51Cv4 %>% 
  filter(allele_count_african_african_american==0, allele_count_remaining==0, allele_count_admixed_american==0, allele_count_ashkenazi_jewish==0, allele_count_east_asian!=0, allele_count_european_finnish==0, allele_count_european_non_finnish==0 , allele_count_south_asian==0, allele_count_middle_eastern==0, allele_count_amish==0) %>% 
  count(chromosome) %>% 
  inner_join(Total_rad51c_eas, by="chromosome") %>% 
  mutate(n.z=n.y-n.x) %>% 
  rename("total" = "n.y", "unique" = "n.x", "shared" = "n.z") %>% 
  mutate(Population="EAS") %>% 
  select(Population, total, shared, unique) -> Unique_rad51c_eas

RAD51Cv4 %>% 
  filter(allele_count_south_asian!=0) %>% 
  count(chromosome) -> Total_rad51c_sas
RAD51Cv4 %>% 
  filter(allele_count_african_african_american==0, allele_count_remaining==0, allele_count_admixed_american==0, allele_count_ashkenazi_jewish==0, allele_count_east_asian==0, allele_count_european_finnish==0, allele_count_european_non_finnish==0 , allele_count_south_asian!=0, allele_count_middle_eastern==0, allele_count_amish==0) %>% 
  count(chromosome) %>% 
  inner_join(Total_rad51c_sas, by="chromosome") %>% 
  mutate(n.z=n.y-n.x) %>% 
  rename("total" = "n.y", "unique" = "n.x", "shared" = "n.z") %>% 
  mutate(Population="SAS") %>% 
  select(Population, total, shared, unique) -> Unique_rad51c_sas

full_join(Unique_rad51c_afr, Unique_rad51c_amr) %>% full_join(Unique_rad51c_nfe) %>% full_join(Unique_rad51c_eas) %>% full_join(Unique_rad51c_sas) %>%
  select(-total) %>% 
  pivot_longer(cols = c(unique, shared), names_to = "Characteristics", values_to = "Values") %>%
  ggplot(aes(x=Population, y=Values, fill=Characteristics)) + geom_col(position="dodge") + geom_text(aes(label = Values), position = position_dodge(1)) + labs(title= "Comparison of shared and unique variants between populations for the RAD51C", y="Number of variants") + xlim("NFE", "AFR", "AMR", "EAS", "SAS") + 
...
ggsave(here("output/v4/Population/RAD51C_pop.png"), width = 8, height = 6, dpi = 300)
```
