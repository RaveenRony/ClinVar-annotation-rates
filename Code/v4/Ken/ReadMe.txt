...
For the scores given by Ken, all variants were annotated in hgvs.g format using GnomAD IDs after the variants were converted from GRCh38 build to GRCh37 build. After the output was received, the files were loaded and used to analyse and create PCAs using data from dbNSFP rankscores aggregated by MyVariant for each gene.
