...
Singleton data analysis
```{r}
singletons_unr <- function(df) {df %>% filter(clin_var_clinical_significance == "Unreported") %>% filter(allele_count %in% c(2, 3, 4, 5)) %>% count(gnom_ad_id) %>% nrow()}

singleton_data <- gnomadv4_count %>% 
  unite(gene_pop, gene, population) %>% 
  nest(data = -gene_pop) %>% 
  mutate(number = map_dbl(data, singletons_unr)) %>% 
  select(gene_pop, number) %>% 
  separate(gene_pop, into = c("gene", "population")) 

singleton_data %>% 
  pivot_wider(names_from = population, values_from = number) %>% 
  write.csv(here("output/v4/csv files/singletons.csv"), row.names = F) #Number of unreported variants with allele counts 2-5 in each gene and population
...
singleton_data %>% 
  ...
  xlim("NFE", "AFR","AMR", "EAS", "SAS") +
  labs(x="Population")
ggsave(here("output/v4/singleton_unreported_1B.png"), dpi=300, width=8, height=6) #Total number of unreported variants per gene and per population which have an allele count of 2-5

singleton_all <- function(df) {df %>% filter(allele_count %in% c(2, 3, 4, 5)) %>% count(gnom_ad_id) %>% nrow()}

singleton_all_data <- gnomadv4_count %>% 
  unite(gene_pop, gene, population) %>% 
  nest(data = -gene_pop) %>% 
  mutate(number = map_dbl(data, singleton_all)) %>% 
  select(gene_pop, number) %>% 
  separate(gene_pop, into = c("gene", "population")) 

singleton_all_data %>% 
  pivot_wider(names_from = population, values_from = number) %>% 
  write.csv(here("output/v4/csv files/singletons_all_data.csv"), row.names = F) #Number of all variants with allele counts 2-5 in each gene and population
...
singleton_all_data %>% 
  ...
  xlim("NFE", "AFR","AMR", "EAS", "SAS") +
  labs(x="Population")
ggsave(here("output/v4/singleton_all_1B.png"), dpi=300, width=8, height=6) #Total number of all variants per gene and per population which have an allele count of 2-5
...
gnomadv4_count %>% 
  ...
  xlim("NFE", "AFR", "AMR", "EAS", "SAS") +
  labs(x="Population", y = "Percentage of variants")
ggsave(here("output/v4/singleton_all_1F.png"), height = 6, width = 8, dpi = 300)#Percentage of all variants which have an allele count of 2-5 in each ClinVar annotations per population.
...
gnomadv4_singleton_total <- gnomadv4_count %>% filter(allele_count %in% c(2, 3, 4, 5)) %>% count(population) %>% rename("total" = "n")
...
gnomadv4_count %>%
  ...
  filter(!grepl("Unreported", clin_var_clinical_significance, ignore.case = TRUE)) %>% 
  summarise(in_clinvar = sum(n, na.rm = TRUE)) %>% 
  full_join(gnomadv4_singleton_total) %>% 
