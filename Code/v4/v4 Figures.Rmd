...
Loading of gene data from gnomAD v2
```{r}
ATM <- clean_names(read.csv(here("data/ATM.csv"))) %>% mutate(gene = "ATM",.before = chromosome)
BARD1 <- clean_names(read.csv(here("data/BARD1.csv"))) %>% mutate(gene = "BARD1",.before = chromosome)
BRCA1 <- clean_names(read.csv(here("data/BRCA1.csv"))) %>% mutate(gene = "BRCA1",.before = chromosome)
BRCA2 <- clean_names(read.csv(here("data/BRCA2.csv"))) %>% mutate(gene = "BRCA2",.before = chromosome)
CHEK2 <- clean_names(read.csv(here("data/CHEK2.csv"))) %>% mutate(gene = "CHEK2",.before = chromosome)
PALB2 <- clean_names(read.csv(here("data/PALB2.csv"))) %>% mutate(gene = "PALB2",.before = chromosome)
RAD51C <- clean_names(read.csv(here("data/RAD51C.csv"))) %>% mutate(gene = "RAD51C",.before = chromosome)
RAD51D <- clean_names(read.csv(here("data/RAD51D.csv"))) %>% mutate(gene = "RAD51D",.before = chromosome)
TP53 <- clean_names(read.csv(here("data/TP53.csv"))) %>% mutate(gene = "TP53",.before = chromosome)
...
gnomad <- full_join(ATM, BARD1) %>% full_join(BRCA1) %>% full_join(BRCA2) %>% full_join(CHEK2) %>% full_join(PALB2) %>% full_join(RAD51C) %>% full_join(RAD51D) %>% full_join(TP53)
...
gnomad_count <-  full_join(gnomad_afr, gnomad_amr) %>% full_join(gnomad_nfe) %>% full_join(gnomad_eas) %>% full_join(gnomad_sas)

gnomad_total <- gnomad_count %>% group_by(gene) %>% count(population) %>% rename("total"="n")
```
...
gnomadv4_count %>% 
  ...
  xlim("Total", "NFE", "AFR","AMR", "EAS", "SAS") +
  labs(x="Population")
...
gnomadv4_count %>% 
  ...
  theme(axis.text.y = element_text(size = 15),
        ...
        legend.text = element_text(size = 15),
        legend.title = element_blank()) +
  xlim("Total", "ATM", "BARD1", "BRCA1", "BRCA2", "CHEK2","PALB2", "RAD51C", "RAD51D", "TP53") +
...
gnomadv4_count %>% 
  mutate(count_range = case_when(count_range == "0-1" ~ "1", T ~ count_range)) %>% 
  ...
  bind_rows(gnomadv4_count %>% 
            mutate(count_range = case_when(count_range == "0-1" ~ "1", T ~ count_range)) %>% 
            unite(test, gene, count_range, sep = " ") %>% 
            group_by(test) %>% 
            summarise(n = n()) %>% 
            separate(test, into = c("gene", "count_range"), sep = " ") %>% 
            mutate(clin_var_clinical_significance = "Total")) %>% 
  unite(test, gene, clin_var_clinical_significance, sep = " ") %>% 
  group_by(test) %>%
  mutate(normalised = round((n / sum(n)) * 100, 2)) %>% 
  separate(test, into = c("gene", "clin_var_clinical_significance"), sep = " ") %>% 
  ggplot(aes(fill=factor(count_range, c("1", "2-3", "4-10", "11-100", "100+")) , x = clin_var_clinical_significance, y = normalised)) + 
    facet_wrap(~gene) +
    ...
    xlim("Total", "Benign", "Pathogenic", "Uncertain", "Unreported") +
...
fig2A_bonferroi_annot <- fig2A_data %>% 
  filter(Version == "v4") %>% 
  unite(test, gene, Population) %>% 
  filter(test %in% c("ATM_AFR", "BRCA1_AMR", "BRCA2_AMR", "TP53_EAS", "ATM_SAS", "BRCA2_SAS", "PALB2_SAS")) %>% 
  separate(test, into = c("gene", "Population"))

fig2A_nominal_annot <- fig2A_data %>% 
  filter(Version == "v4") %>% 
  unite(test, gene, Population) %>% 
  filter(test %in% c("BRCA1_AFR", "BRCA2_AFR","PALB2_AFR", "RAD51C_AMR", "BARD1_SAS", "BRCA1_SAS", "CHEK2_SAS", "TP53_SAS")) %>% 
  separate(test, into = c("gene", "Population"))

fig2A_data %>% 
  ggplot(aes(x=Population, y=Percentage)) +
  facet_wrap(~gene) +
  geom_point(aes(shape=Population), size = 2.5) + 
  geom_line(aes(group = Version, colour = Version), linetype = 2, size = 0.7) +
  geom_text(data = fig2A_bonferroi_annot, aes(x = Population, y = Percentage, label = "*", colour = Version), vjust = 2, size = 6, inherit.aes = F, show.legend = F) +
  geom_text(data = fig2A_nominal_annot, aes(x = Population, y = Percentage, label = "○", colour = Version), vjust = 2, size = 6, inherit.aes = F, show.legend = F) +
  geom_point(data = fig2A_bonferroi_annot, aes(fill = "Adjusted"), colour = "transparent") +
  geom_point(data = fig2A_nominal_annot, aes(fill = "Nominal"), colour = "transparent") +
  theme_bw() + 
  theme(axis.text.x = element_text(size = 15, angle = 35, hjust = 1),
        ...
        legend.title = element_text(size = 18),
        legend.text = element_text(size = 15)) + 
  guides(shape = guide_legend(title = "Population", order = 2),
         colour = guide_legend(title = "Version", order = 1),
         fill  = guide_legend(title = "Significance", order = 3, override.aes = list(shape = c(8, 1), size = 4, colour = c("turquoise3", "turquoise3")))) +
  ylim(0, 100) + xlim("NFE", "AFR", "AMR", "EAS", "SAS")
ggsave(here("output/v4/fig2A.png"), width = 8, height = 6, dpi = 300)

gnomadv4_combine_total <- gnomadv4_count %>% filter(vep_annotation=="frameshift_variant"|vep_annotation=="splice_site_variant"|vep_annotation=="stop_gained") %>% group_by(gene) %>% count(population) %>% rename("total" = "n")

fig2B_data <- gnomadv4_count %>%
  filter(vep_annotation=="frameshift_variant"|vep_annotation=="splice_site_variant"|vep_annotation=="stop_gained") %>% 
  unite(test, gene, population, sep = " ") %>% 
  group_by(test) %>% 
  count(clin_var_clinical_significance) %>% 
  ungroup() %>% 
...
fig2B_nominal_annot <- fig2B_data %>%  
  unite(test, gene, population) %>% 
  filter(test %in% c("ATM_SAS", "BRCA2_SAS", "TP53_SAS")) %>% 
  separate(test, into = c("gene", "population"))

fig2B_data %>% 
  ggplot(aes(x=population,y=percent_in_clinvar, shape=population)) + geom_point(size = 2.5) + 
  geom_line(aes(group = gene), colour = "black", linetype = 2, size = 0.6) +
  geom_text(data = fig2B_nominal_annot, aes(x = population, y = percent_in_clinvar, label = "○"), vjust = -0.8, size = 6, inherit.aes = FALSE) +
  geom_point(data = fig2B_nominal_annot, aes(fill = "Nominal"), colour = "transparent") +
  facet_wrap(~gene) +
  theme_bw() + 
  theme(axis.text.x = element_text(size = 15, angle = 35, hjust = 1),
        axis.title.x = element_text(size = 20),
        ...
        strip.text = element_text(size = 10.5)) + 
  ylim(0, 100) + xlim("NFE", "AFR", "AMR", "EAS", "SAS") +
  guides(shape = guide_legend(title = "Population", order = 1),
         fill  = guide_legend(title = "Significance", order = 2, override.aes = list(shape = 1, size = 4, colour = "black"))) +
...
Variants with very low frequency analysis
```{r}
var_vlow_unr <- function(df) {df %>% filter(clin_var_clinical_significance == "Unreported") %>% filter(allele_count %in% c(2, 3, 4, 5)) %>% count(gnom_ad_id) %>% nrow()}

var_vlow_data <- gnomadv4_count %>% 
  unite(gene_pop, gene, population) %>% 
  nest(data = -gene_pop) %>% 
  mutate(number = map_dbl(data, var_vlow_unr)) %>% 
  select(gene_pop, number) %>% 
  separate(gene_pop, into = c("gene", "population")) 

singleton_data %>% 
  pivot_wider(names_from = population, values_from = number) %>% 
  write.csv(here("output/v4/csv files/var_vlow_unreported.csv"), row.names = F) #Number of unreported variants with allele counts 2-5 in each gene and population

var_vlow_data %>% 
  ggplot(aes(x= population, y = number, fill = gene)) + geom_col(colour = "#000000") + 
  theme_bw() +
  theme(axis.title.y = element_blank(),
        legend.title = element_blank(),
        axis.text.y = element_text(size = 15),
  ...
  labs(x="Population")
ggsave(here("output/v4/var_vlow_unreported_1B.png"), dpi=300, width=8, height=6) #Total number of unreported variants per gene and per population which have an allele count of 2-5

var_vlow_all <- function(df) {df %>% filter(allele_count %in% c(2, 3, 4, 5)) %>% count(gnom_ad_id) %>% nrow()}

var_vlow_all_data <- gnomadv4_count %>% 
  unite(gene_pop, gene, population) %>% 
  nest(data = -gene_pop) %>% 
  mutate(number = map_dbl(data, var_vlow_all)) %>% 
  select(gene_pop, number) %>% 
  separate(gene_pop, into = c("gene", "population")) 

var_vlow_all_data %>% 
  pivot_wider(names_from = population, values_from = number) %>% 
  write.csv(here("output/v4/csv files/var_vlow_all_data.csv"), row.names = F) #Number of all variants with allele counts 2-5 in each gene and population
...
var_vlow_all_data %>% 
  ...
  xlim("NFE", "AFR","AMR", "EAS", "SAS") +
  labs(x="Population")
ggsave(here("output/v4/var_vlow_all_1B.png"), dpi=300, width=8, height=6) #Total number of all variants per gene and per population which have an allele count of 2-5

gnomadv4_count %>% 
  filter(allele_count %in% c(2, 3, 4, 5)) %>% 
  group_by(population) %>% 
  count(clin_var_clinical_significance) %>% 
  mutate(percent = round((n/sum(n))*100,2)) %>% 
  ggplot(aes(x=population, y=percent, fill=clin_var_clinical_significance)) + 
  ...
  xlim("NFE", "AFR", "AMR", "EAS", "SAS") +
  labs(x="Population", y = "Percentage of variants")
ggsave(here("output/v4/var_vlow_all_1F.png"), height = 6, width = 8, dpi = 300)#Percentage of all variants which have an allele count of 2-5 in each ClinVar annotations per population.

gnomadv4_unr_total <- gnomadv4_count %>% filter(clin_var_clinical_significance == "Unreported") %>% count(population) %>% rename("total" = "n")

gnomadv4_count %>%
  filter(clin_var_clinical_significance == "Unreported") %>% 
  filter(allele_count %in% c(2, 3, 4, 5)) %>%
  count(population) %>% 
  full_join(gnomadv4_unr_total) %>% 
  replace_na(list(n = 0)) %>% 
  mutate(percent = round((n/total)*100,2),
         gene = "All") %>% 
  ggplot(aes(x=population,y=percent, shape=population)) + geom_point(size = 2.5) + 
  geom_line(aes(group = gene), colour = "black", linetype = 2, size = 0.6) +
  theme_bw() + 
  theme(axis.text.x = element_text(size = 15, angle = 35, hjust = 1),
        axis.title.x = element_text(size = 20),
        ...
        legend.text = element_text(size = 15),
        strip.text = element_text(size = 10.5)) + 
  ylim(0, 20) + xlim("NFE", "AFR", "AMR", "EAS", "SAS") +
  labs(x = "Population", y = "Percentage", shape = "Population")
ggsave(here("output/v4/percent_of_unreported_2-5_variants.png"), width = 8, height = 6, dpi = 300) #Line graph depicting percentage of unreported variants that has allele count 2-5

gnomadv4_var_vlow_total <- gnomadv4_count %>% filter(allele_count %in% c(2, 3, 4, 5)) %>% count(population) %>% rename("total" = "n")
...
gnomadv4_count %>%
  ...
  count(clin_var_clinical_significance) %>% 
  ungroup() %>% 
  complete(population, clin_var_clinical_significance, fill = list(n = 0)) %>% 
  group_by(population) %>% 
  filter(!grepl("Unreported", clin_var_clinical_significance, ignore.case = TRUE)) %>% 
  summarise(in_clinvar = sum(n, na.rm = TRUE)) %>% 
  full_join(gnomadv4_var_vlow_total) %>% 
  mutate(percent = round((in_clinvar/total)*100,2),
         gene = "All") %>% 
  ggplot(aes(x=population,y=percent, shape=population)) + geom_point(size = 2.5) + 
  geom_line(aes(group = gene), colour = "black", linetype = 2, size = 0.6) +
  theme_bw() + 
  theme(axis.text.x = element_text(size = 15, angle = 35, hjust = 1),
        ...
        legend.text = element_text(size = 15),
        strip.text = element_text(size = 10.5)) + 
  ylim(50, 100) + xlim("NFE", "AFR", "AMR", "EAS", "SAS") +
  labs(x = "Population", y = "Percentage", shape = "Population")
ggsave(here("output/v4/percent_of_2-5_variants_in_clinvar.png"), width = 8, height = 6, dpi = 300) #Line graph depicting percentage of variants that has allele count 2-5 in ClinVar
```
...
pca_v4 %>% 
  ...
  select(-c(chromosome, position, end, reference, alternate, vep_annotation, allele_count, count_range, hg38, hg19)) %>% 
  ...
  unite(test, gene, hgvsg, clin_var_clinical_significance, population, sep = ", ") -> gnomadv4_population_assay

transposed_gnomadv4_pop_assay <- as.data.frame(t(gnomadv4_population_assay))

col_to_remv4 <- colSums(is.na(gnomadv4_population_assay))
gnomadv4_pca_pop <- rbind(gnomadv4_population_assay, col_to_rem)

col_tremv4<- colSums(is.na(transposed_gnomadv4_pop_assay))
transposed_gnomadv4_pop_pca <- rbind(transposed_gnomadv4_pop_assay, col_tremv4)
rownames_to_column(transposed_gnomadv4_pca, var = "rankscore")
transposed_gnomadv4_pop_pca <- transposed_gnomadv4_pop_pca %>% select(which(.[35, ] == 0|.[35, ] == 1|.[35, ] == 4))
transposed_gnomadv4_pop_pca <- transposed_gnomadv4_pop_pca[-35,]
transposed_gnomadv4_pop_pca %>% write.table(file = here("data/PCA_pop_assay.txt"), row.names = F, col.names = F)

gnomadv4_pop_pca <- read.table(here("data/PCA_pop_assay.txt"))
colnames(gnomadv4_pop_pca) <- gnomadv4_pop_pca[1, ]
gnomadv4_pop_pca <- gnomadv4_pop_pca[-1, ]
gnomadv4_pop_pca[] <- lapply(gnomadv4_pop_pca, as.numeric)
gnomadv4_pop_pca <- gnomadv4_pop_pca %>% drop_na()
#9799/10164 missense variants having 24 rankscores

pca_pop_result <- prcomp(gnomadv4_pop_pca, center = TRUE, scale. = TRUE)
fviz_eig(pca_pop_result, addlabels = T)

pca_pop_data <- as.data.frame(pca_pop_result[["rotation"]])
pca_pop_data %>% write.table(here("data/PCA_pop.txt"), col.names = F)
pca_pop_gnomadv4 <- read.table(here("data/PCA_pop.txt"))

pca_pop_gnomadv4_modified_rank <- pca_pop_gnomadv4 %>%
  separate(V1, c("gene", "variant", "clin_sig", "population"), sep = ", ") %>%
  mutate(clin_sig = case_when(clin_sig == "NA" ~ "Unreported", clin_sig == "Not_annotated" ~ "Unreported", TRUE ~ clin_sig))

pca_nfe <- pca_pop_gnomadv4_modified_rank %>% filter(population == "NFE")
pca_afr <- pca_pop_gnomadv4_modified_rank %>% filter(population == "AFR")
pca_amr <- pca_pop_gnomadv4_modified_rank %>% filter(population == "AMR")
pca_eas <- pca_pop_gnomadv4_modified_rank %>% filter(population == "EAS")
pca_sas <- pca_pop_gnomadv4_modified_rank %>% filter(population == "SAS")

pca_nfe_plot <- ggplot(pca_nfe, aes(x = V2, y = V3, colour = clin_sig)) +
  scale_colour_manual(values = c("Benign" = "skyblue", "Pathogenic" = "red", "Unreported" = "darkgray", "Uncertain" = "darkgreen")) + theme_bw() +
  labs(x = "Principal Component 1", y = "Principal Component 2") +
  theme(axis.text.y = element_text(size = 15),
        axis.text.x = element_text(size = 15),
        ...
        legend.position = "top") +
  geom_point() +
  geom_point(data = filter(pca_nfe, clin_sig=="Unreported")) +
  geom_point(data = filter(pca_nfe, clin_sig=="Uncertain")) +
  geom_point(data = filter(pca_nfe, clin_sig=="Benign"), size = 2) +
  geom_point(data = filter(pca_nfe, clin_sig=="Pathogenic"), size = 2) + 
  guides(colour = guide_legend(override.aes = list(shape = 15, size = 5)))
ggMarginal(pca_nfe_plot, type = "boxplot", margins = "both", groupFill = T, groupColour = T, xparams = )
ggsave(here("output/v4/PCA/nfe-rankscores.png"), width = 8, height = 6, dpi = 300)

pca_afr_plot <- ggplot(pca_afr, aes(x = V2, y = V3, colour = clin_sig)) +
  scale_colour_manual(values = c("Benign" = "skyblue", "Pathogenic" = "red", "Unreported" = "darkgray", "Uncertain" = "darkgreen")) + theme_bw() +
  labs(x = "Principal Component 1", y = "Principal Component 2") +
  theme(axis.text.y = element_text(size = 15),
        axis.text.x = element_text(size = 15),
        ...
        legend.position = "top") +
  geom_point() +
  geom_point(data = filter(pca_afr, clin_sig=="Unreported")) +
  geom_point(data = filter(pca_afr, clin_sig=="Uncertain")) +
  geom_point(data = filter(pca_afr, clin_sig=="Benign"), size = 2) +
  geom_point(data = filter(pca_afr, clin_sig=="Pathogenic"), size = 2) + 
  guides(colour = guide_legend(override.aes = list(shape = 15, size = 5)))
ggMarginal(pca_afr_plot, type = "boxplot", margins = "both", groupFill = T, groupColour = T, xparams = )
ggsave(here("output/v4/PCA/afr-rankscores.png"), width = 8, height = 6, dpi = 300)

pca_amr_plot <- ggplot(pca_amr, aes(x = V2, y = V3, colour = clin_sig)) +
  scale_colour_manual(values = c("Benign" = "skyblue", "Pathogenic" = "red", "Unreported" = "darkgray", "Uncertain" = "darkgreen")) + theme_bw() +
  labs(x = "Principal Component 1", y = "Principal Component 2") +
  theme(axis.text.y = element_text(size = 15),
        axis.text.x = element_text(size = 15),
        ...
        legend.position = "top") +
  geom_point() +
  geom_point(data = filter(pca_amr, clin_sig=="Unreported")) +
  geom_point(data = filter(pca_amr, clin_sig=="Uncertain")) +
  geom_point(data = filter(pca_amr, clin_sig=="Benign"), size = 2) +
  geom_point(data = filter(pca_amr, clin_sig=="Pathogenic"), size = 2) + 
  guides(colour = guide_legend(override.aes = list(shape = 15, size = 5)))
ggMarginal(pca_amr_plot, type = "boxplot", margins = "both", groupFill = T, groupColour = T, xparams = )
ggsave(here("output/v4/PCA/amr-rankscores.png"), width = 8, height = 6, dpi = 300)

pca_eas_plot <- ggplot(pca_eas, aes(x = V2, y = V3, colour = clin_sig)) +
  scale_colour_manual(values = c("Benign" = "skyblue", "Pathogenic" = "red", "Unreported" = "darkgray", "Uncertain" = "darkgreen")) + theme_bw() +
  labs(x = "Principal Component 1", y = "Principal Component 2") +
  theme(axis.text.y = element_text(size = 15),
        axis.text.x = element_text(size = 15),
        ...
        legend.position = "top") +
  geom_point() +
  geom_point(data = filter(pca_eas, clin_sig=="Unreported")) +
  geom_point(data = filter(pca_eas, clin_sig=="Uncertain")) +
  geom_point(data = filter(pca_eas, clin_sig=="Benign"), size = 2) +
  geom_point(data = filter(pca_eas, clin_sig=="Pathogenic"), size = 2) + 
  guides(colour = guide_legend(override.aes = list(shape = 15, size = 5)))
ggMarginal(pca_eas_plot, type = "boxplot", margins = "both", groupFill = T, groupColour = T, xparams = )
ggsave(here("output/v4/PCA/eas-rankscores.png"), width = 8, height = 6, dpi = 300)

pca_sas_plot <- ggplot(pca_sas, aes(x = V2, y = V3, colour = clin_sig)) +
  scale_colour_manual(values = c("Benign" = "skyblue", "Pathogenic" = "red", "Unreported" = "darkgray", "Uncertain" = "darkgreen")) + theme_bw() +
  labs(x = "Principal Component 1", y = "Principal Component 2") +
  theme(axis.text.y = element_text(size = 15),
        axis.text.x = element_text(size = 15),
        ...
        legend.position = "top") +
  geom_point() +
  geom_point(data = filter(pca_sas, clin_sig=="Unreported")) +
  geom_point(data = filter(pca_sas, clin_sig=="Uncertain")) +
  geom_point(data = filter(pca_sas, clin_sig=="Benign"), size = 2) +
  geom_point(data = filter(pca_sas, clin_sig=="Pathogenic"), size = 2) + 
  guides(colour = guide_legend(override.aes = list(shape = 15, size = 5)))
ggMarginal(pca_sas_plot, type = "boxplot", margins = "both", groupFill = T, groupColour = T, xparams = )
ggsave(here("output/v4/PCA/sas-rankscores.png"), width = 8, height = 6, dpi = 300)
```
...
gnomad_count %>% 
  ...
  facet_wrap(~gene) +
  geom_point(size = 2.5) + geom_line(aes(group = Version, colour=Version), linetype = 2, size = 0.7) +
  theme_bw() + 
  theme(axis.text.x = element_text(size = 15, angle = 35, hjust = 1),
        axis.title.x = element_text(size = 20),
        axis.title.y = element_text(size = 20),
...
Fisher test for combination of Inframe, Frameshift, Splice Site and Stop Gained variants
```{r}
...
private_variants <- gnomadv4_count %>% count(population)
population_data_normalised <- private_variants %>% 
  add_column(total_individuals = c(37545, 30109, 22448, 622057, 45546)) %>% #Number of individuals from gnomAD v4.0 dataset
  mutate(rate_per_10000_individuals = (n/total_individuals)*10000) %>% 
  rename("population_private_variants" = "n") %>% 
  select(population, total_individuals, population_private_variants, rate_per_10000_individuals) #Population Normalisation

gnomadv4_count %>% 
  select(gnom_ad_id, population, vep_annotation, clin_var_clinical_significance) %>% 
  group_by(population) %>% 
  count(vep_annotation) %>% 
  pivot_wider(names_from = vep_annotation, values_from = n) %>% 
  replace_na(list(stop_lost = 0)) %>% 
  write.csv(here("output/v4/csv files/variant_by_ancestry.csv")) #Number of variants of each type in each population
...
gnomadv4_count %>% 
  ...
  write.csv(here("output/v4/csv files/pathogenic_variants.csv")) #Number of pathogenic variants in each population
```
```
