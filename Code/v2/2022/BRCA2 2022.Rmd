gnomAD 2022 - BRCA2 - Population specific variant table & Comparison of unique and shared variants within population
...
Loading data
```{r}
brca2_2 <- clean_names(read_csv(here("2022 data/BRCA2 2022.csv")))

brca2_2_african_unique <- function() {brca2_2 %>% 
...
brca2_2_latino_unique <- function() {brca2_2 %>% 
   filter(allele_count_african_african_american==0 & allele_count_other==0 & allele_count_latino_admixed_american!=0 & allele_count_ashkenazi_jewish==0 & allele_count_east_asian==0 & allele_count_european_finnish==0 & allele_count_european_non_finnish==0 , allele_count_south_asian==0)}

brca2_2_european_unique <- function() {brca2_2 %>% 
   filter(allele_count_african_african_american==0 & allele_count_other==0 & allele_count_latino_admixed_american==0 & allele_count_ashkenazi_jewish==0 & allele_count_east_asian==0 & allele_count_european_finnish==0 & allele_count_european_non_finnish!=0 , allele_count_south_asian==0)}

brca2_2_east_asian_unique <- function() {brca2_2 %>% 
   filter(allele_count_african_african_american==0 & allele_count_other==0 & allele_count_latino_admixed_american==0 & allele_count_ashkenazi_jewish==0 & allele_count_east_asian!=0 & allele_count_european_finnish==0 & allele_count_european_non_finnish==0 , allele_count_south_asian==0)}

brca2_2_south_asian_unique <- function() {brca2_2 %>% 
   filter(allele_count_african_african_american==0 & allele_count_other==0 & allele_count_latino_admixed_american==0 & allele_count_ashkenazi_jewish==0 & allele_count_east_asian==0 & allele_count_european_finnish==0 & allele_count_european_non_finnish==0 , allele_count_south_asian!=0)}
...
African American
```{r}
brca2_2_african_table <- brca2_2_african_unique() %>% 
...
Latino
```{r}
brca2_2_latino_table <- brca2_2_latino_unique() %>% 
  group_by(vep_annotation) %>% 
  mutate(clin_var_clinical_significance= case_when(clin_var_clinical_significance %in% Pathogenic ~ "Pathogenic", clin_var_clinical_significance %in% Uncertain ~ "Uncertain", clin_var_clinical_significance %in% Benign ~ "Benign", clin_var_clinical_significance=="not provided" ~ NA, TRUE~clin_var_clinical_significance)) %>% mutate(vep_annotation=case_when(vep_annotation %in% Splice ~ "splice_site_variant", vep_annotation %in% Inframe ~ "inframe_variant", vep_annotation%in%Stop ~ "stop_variant", TRUE ~ vep_annotation)) %>% 
  count(clin_var_clinical_significance) %>% mutate(Total= sum(n)) %>% 
  mutate(Percent_in_ClinVar=round(prop.table(n),4)*100) %>% 
...
European
```{r}
brca2_2_european_table <- brca2_2_european_unique() %>% 
  group_by(vep_annotation) %>% 
  mutate(clin_var_clinical_significance= case_when(clin_var_clinical_significance %in% Pathogenic ~ "Pathogenic", clin_var_clinical_significance %in% Uncertain ~ "Uncertain", clin_var_clinical_significance %in% Benign ~ "Benign", clin_var_clinical_significance=="not provided" ~ NA, TRUE~clin_var_clinical_significance)) %>% mutate(vep_annotation=case_when(vep_annotation %in% Splice ~ "splice_site_variant", vep_annotation %in% Inframe ~ "inframe_variant", vep_annotation%in%Stop ~ "stop_variant", TRUE ~ vep_annotation)) %>% 
  count(clin_var_clinical_significance) %>% mutate(Total= sum(n)) %>% 
  mutate(Percent_in_ClinVar=round(prop.table(n),4)*100) %>% 
...
East asian
```{r}
brca2_2_east_asian_table <- brca2_2_east_asian_unique() %>% 
  group_by(vep_annotation) %>% 
  mutate(clin_var_clinical_significance= case_when(clin_var_clinical_significance %in% Pathogenic ~ "Pathogenic", clin_var_clinical_significance %in% Uncertain ~ "Uncertain", clin_var_clinical_significance %in% Benign ~ "Benign", clin_var_clinical_significance=="not provided" ~ NA, TRUE~clin_var_clinical_significance)) %>% mutate(vep_annotation=case_when(vep_annotation %in% Splice ~ "splice_site_variant", vep_annotation %in% Inframe ~ "inframe_variant", vep_annotation%in%Stop ~ "stop_variant", TRUE ~ vep_annotation)) %>% 
  count(clin_var_clinical_significance) %>% mutate(Total= sum(n)) %>% 
  mutate(Percent_in_ClinVar=round(prop.table(n),4)*100) %>% 
...
South asian
```{r}
brca2_2_south_asian_table <- brca2_2_south_asian_unique() %>% 
  group_by(vep_annotation) %>% 
  mutate(clin_var_clinical_significance= case_when(clin_var_clinical_significance %in% Pathogenic ~ "Pathogenic", clin_var_clinical_significance %in% Uncertain ~ "Uncertain", clin_var_clinical_significance %in% Benign ~ "Benign", clin_var_clinical_significance=="not provided" ~ NA, TRUE~clin_var_clinical_significance)) %>% mutate(vep_annotation=case_when(vep_annotation %in% Splice ~ "splice_site_variant", vep_annotation %in% Inframe ~ "inframe_variant", vep_annotation%in%Stop ~ "stop_variant", TRUE ~ vep_annotation)) %>% 
  count(clin_var_clinical_significance) %>% mutate(Total= sum(n)) %>% 
  mutate(Percent_in_ClinVar=round(prop.table(n),4)*100) %>% 
...
brca2_2 comparison of variants between population
```{r}
brca2_2 %>% filter(allele_count_african_african_american!=0) %>% count(chromosome)->Total_brca2_2_african
brca2_2_african_unique() %>% count(chromosome) -> brca2_2_african_uni
inner_join(Total_brca2_2_african,brca2_2_african_uni, by="chromosome") %>% mutate(n.z=n.x-n.y) %>% rename(Population=chromosome) -> African
African["Population"] <-  "African"

brca2_2 %>% filter(allele_count_latino_admixed_american!=0) %>% count(chromosome)->Total_brca2_2_latino
brca2_2_latino_unique() %>% count(chromosome) -> brca2_2_latino_uni
inner_join(Total_brca2_2_latino,brca2_2_latino_uni, by="chromosome") %>% mutate(n.z=n.x-n.y) %>% rename(Population=chromosome) -> Latino
Latino["Population"] <-  "Latino"

brca2_2 %>% filter(allele_count_european_non_finnish!=0) %>% count(chromosome)->Total_brca2_2_europe
brca2_2_european_unique() %>% count(chromosome)->brca2_2_europe_uni
inner_join(Total_brca2_2_europe,brca2_2_europe_uni, by="chromosome") %>% mutate(n.z=n.x-n.y) %>% rename(Population=chromosome) -> Europe
Europe["Population"] <-  "Europe"

brca2_2 %>% filter(allele_count_east_asian!=0) %>% count(chromosome)->Total_brca2_2_east_asia
brca2_2_east_asian_unique() %>% count(chromosome)->brca2_2_east_asia_uni
inner_join(Total_brca2_2_east_asia,brca2_2_east_asia_uni, by="chromosome") %>% mutate(n.z=n.x-n.y) %>% rename(Population=chromosome) -> East_asia
East_asia["Population"] <-  "East_asia"

brca2_2 %>% filter(allele_count_south_asian!=0) %>% count(chromosome) -> Total_brca2_2_south_asia
brca2_2_south_asian_unique() %>% count(chromosome)->brca2_2_south_asia_uni
inner_join(Total_brca2_2_south_asia,brca2_2_south_asia_uni, by="chromosome") %>% mutate(n.z=n.x-n.y) %>% rename(Population=chromosome) -> South_asia
South_asia["Population"] <-  "South_asia"

full_join(African, Latino) -> brca2_2TUS
full_join(brca2_2TUS,Europe) -> brca2_2TUS
full_join(brca2_2TUS,East_asia) -> brca2_2TUS
full_join(brca2_2TUS, South_asia) -> brca2_2TUS
brca2_2TUS %>% rename(Total=n.x, Unique=n.y, Shared=n.z) %>% select(-Total) %>% pivot_longer(cols = c(Unique, Shared), names_to = "Characteristics", values_to = "Values") %>% 
...
for the brca2_2 gene", y="Number of variants")

ggsave(here("output/brca2_2_pop.png"), width = 8, height = 6, dpi = 300)
```
