gnomAD 2022 - BARD1 - Population specific variant table & Comparison of unique and shared variants within population
...
Loading data
```{r}
bard1_2 <- clean_names(read_csv(here("2022 data/BARD1 2022.csv")))

bard1_2_african_unique <- function() {bard1_2 %>% 
...
bard1_2_latino_unique <- function() {bard1_2 %>% 
   filter(allele_count_african_african_american==0 & allele_count_other==0 & allele_count_latino_admixed_american!=0 & allele_count_ashkenazi_jewish==0 & allele_count_east_asian==0 & allele_count_european_finnish==0 & allele_count_european_non_finnish==0 , allele_count_south_asian==0)}

bard1_2_european_unique <- function() {bard1_2 %>% 
   filter(allele_count_african_african_american==0 & allele_count_other==0 & allele_count_latino_admixed_american==0 & allele_count_ashkenazi_jewish==0 & allele_count_east_asian==0 & allele_count_european_finnish==0 & allele_count_european_non_finnish!=0 , allele_count_south_asian==0)}

bard1_2_east_asian_unique <- function() {bard1_2 %>% 
   filter(allele_count_african_african_american==0 & allele_count_other==0 & allele_count_latino_admixed_american==0 & allele_count_ashkenazi_jewish==0 & allele_count_east_asian!=0 & allele_count_european_finnish==0 & allele_count_european_non_finnish==0 , allele_count_south_asian==0)}

bard1_2_south_asian_unique <- function() {bard1_2 %>% 
   filter(allele_count_african_african_american==0 & allele_count_other==0 & allele_count_latino_admixed_american==0 & allele_count_ashkenazi_jewish==0 & allele_count_east_asian==0 & allele_count_european_finnish==0 & allele_count_european_non_finnish==0 , allele_count_south_asian!=0)}
...
African American
```{r}
bard1_2_african_table <- bard1_2_african_unique() %>% 
...
Latino
```{r}
bard1_2_latino_table <- bard1_2_latino_unique() %>% 
  group_by(vep_annotation) %>% 
  mutate(clin_var_clinical_significance= case_when(clin_var_clinical_significance %in% Pathogenic ~ "Pathogenic", clin_var_clinical_significance %in% Uncertain ~ "Uncertain", clin_var_clinical_significance %in% Benign ~ "Benign", clin_var_clinical_significance=="not provided" ~ NA, TRUE~clin_var_clinical_significance)) %>% mutate(vep_annotation=case_when(vep_annotation %in% Splice ~ "splice_site_variant", vep_annotation %in% Inframe ~ "inframe_variant", vep_annotation%in%Stop ~ "stop_variant", TRUE ~ vep_annotation)) %>% 
  count(clin_var_clinical_significance) %>% mutate(Total= sum(n)) %>% 
  mutate(Percent_in_ClinVar=round(prop.table(n),4)*100) %>% 
...
European
```{r}
bard1_2_european_table <- bard1_2_european_unique() %>% 
  group_by(vep_annotation) %>% 
  mutate(clin_var_clinical_significance= case_when(clin_var_clinical_significance %in% Pathogenic ~ "Pathogenic", clin_var_clinical_significance %in% Uncertain ~ "Uncertain", clin_var_clinical_significance %in% Benign ~ "Benign", clin_var_clinical_significance=="not provided" ~ NA, TRUE~clin_var_clinical_significance)) %>% mutate(vep_annotation=case_when(vep_annotation %in% Splice ~ "splice_site_variant", vep_annotation %in% Inframe ~ "inframe_variant", vep_annotation%in%Stop ~ "stop_variant", TRUE ~ vep_annotation)) %>% 
  count(clin_var_clinical_significance) %>% mutate(Total= sum(n)) %>% 
  mutate(Percent_in_ClinVar=round(prop.table(n),4)*100) %>% 
...
East asian
```{r}
bard1_2_east_asian_table <- bard1_2_east_asian_unique() %>% 
  group_by(vep_annotation) %>% 
  mutate(clin_var_clinical_significance= case_when(clin_var_clinical_significance %in% Pathogenic ~ "Pathogenic", clin_var_clinical_significance %in% Uncertain ~ "Uncertain", clin_var_clinical_significance %in% Benign ~ "Benign", clin_var_clinical_significance=="not provided" ~ NA, TRUE~clin_var_clinical_significance)) %>% mutate(vep_annotation=case_when(vep_annotation %in% Splice ~ "splice_site_variant", vep_annotation %in% Inframe ~ "inframe_variant", vep_annotation%in%Stop ~ "stop_variant", TRUE ~ vep_annotation)) %>% 
  count(clin_var_clinical_significance) %>% mutate(Total= sum(n)) %>% 
  mutate(Percent_in_ClinVar=round(prop.table(n),4)*100) %>% 
...
South asian
```{r}
bard1_2_south_asian_table <- bard1_2_south_asian_unique() %>% 
  group_by(vep_annotation) %>% 
  mutate(clin_var_clinical_significance= case_when(clin_var_clinical_significance %in% Pathogenic ~ "Pathogenic", clin_var_clinical_significance %in% Uncertain ~ "Uncertain", clin_var_clinical_significance %in% Benign ~ "Benign", clin_var_clinical_significance=="not provided" ~ NA, TRUE~clin_var_clinical_significance)) %>% mutate(vep_annotation=case_when(vep_annotation %in% Splice ~ "splice_site_variant", vep_annotation %in% Inframe ~ "inframe_variant", vep_annotation%in%Stop ~ "stop_variant", TRUE ~ vep_annotation)) %>% 
  count(clin_var_clinical_significance) %>% mutate(Total= sum(n)) %>% 
  mutate(Percent_in_ClinVar=round(prop.table(n),4)*100) %>% 
...
bard1_2 comparison of variants between population
```{r}
bard1_2 %>% filter(allele_count_african_african_american!=0) %>% count(chromosome)->Total_bard1_2_african
bard1_2_african_unique() %>% count(chromosome) -> bard1_2_african_uni
inner_join(Total_bard1_2_african,bard1_2_african_uni, by="chromosome") %>% mutate(n.z=n.x-n.y) %>% rename(Population=chromosome) -> African
African["Population"] <-  "African"

bard1_2 %>% filter(allele_count_latino_admixed_american!=0) %>% count(chromosome)->Total_bard1_2_latino
bard1_2_latino_unique() %>% count(chromosome) -> bard1_2_latino_uni
inner_join(Total_bard1_2_latino,bard1_2_latino_uni, by="chromosome") %>% mutate(n.z=n.x-n.y) %>% rename(Population=chromosome) -> Latino
Latino["Population"] <-  "Latino"

bard1_2 %>% filter(allele_count_european_non_finnish!=0) %>% count(chromosome)->Total_bard1_2_europe
bard1_2_european_unique() %>% count(chromosome)->bard1_2_europe_uni
inner_join(Total_bard1_2_europe,bard1_2_europe_uni, by="chromosome") %>% mutate(n.z=n.x-n.y) %>% rename(Population=chromosome) -> Europe
Europe["Population"] <-  "Europe"

bard1_2 %>% filter(allele_count_east_asian!=0) %>% count(chromosome)->Total_bard1_2_east_asia
bard1_2_east_asian_unique() %>% count(chromosome)->bard1_2_east_asia_uni
inner_join(Total_bard1_2_east_asia,bard1_2_east_asia_uni, by="chromosome") %>% mutate(n.z=n.x-n.y) %>% rename(Population=chromosome) -> East_asia
East_asia["Population"] <-  "East_asia"

bard1_2 %>% filter(allele_count_south_asian!=0) %>% count(chromosome) -> Total_bard1_2_south_asia
bard1_2_south_asian_unique() %>% count(chromosome)->bard1_2_south_asia_uni
inner_join(Total_bard1_2_south_asia,bard1_2_south_asia_uni, by="chromosome") %>% mutate(n.z=n.x-n.y) %>% rename(Population=chromosome) -> South_asia
South_asia["Population"] <-  "South_asia"

full_join(African, Latino) -> bard1_2TUS
full_join(bard1_2TUS,Europe) -> bard1_2TUS
full_join(bard1_2TUS,East_asia) -> bard1_2TUS
full_join(bard1_2TUS, South_asia) -> bard1_2TUS
bard1_2TUS %>% rename(Total=n.x, Unique=n.y, Shared=n.z) %>% select(-Total) %>% pivot_longer(cols = c(Unique, Shared), names_to = "Characteristics", values_to = "Values") %>% 
...
for the bard1_2 gene", y="Number of variants")

ggsave(here("output/bard1_2_pop.png"), width = 8, height = 6, dpi = 300)
```