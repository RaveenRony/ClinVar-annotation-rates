gnomAD 2022 - RAD51D - Population specific variant table & Comparison of unique and shared variants within population
...
Loading data
```{r}
rad51d_2 <- clean_names(read_csv(here("2022 data/RAD51D 2022.csv")))

rad51d_2_african_unique <- function() {rad51d_2 %>% 
...
rad51d_2_latino_unique <- function() {rad51d_2 %>% 
   filter(allele_count_african_african_american==0 & allele_count_other==0 & allele_count_latino_admixed_american!=0 & allele_count_ashkenazi_jewish==0 & allele_count_east_asian==0 & allele_count_european_finnish==0 & allele_count_european_non_finnish==0 , allele_count_south_asian==0)}

rad51d_2_european_unique <- function() {rad51d_2 %>% 
   filter(allele_count_african_african_american==0 & allele_count_other==0 & allele_count_latino_admixed_american==0 & allele_count_ashkenazi_jewish==0 & allele_count_east_asian==0 & allele_count_european_finnish==0 & allele_count_european_non_finnish!=0 , allele_count_south_asian==0)}

rad51d_2_east_asian_unique <- function() {rad51d_2 %>% 
   filter(allele_count_african_african_american==0 & allele_count_other==0 & allele_count_latino_admixed_american==0 & allele_count_ashkenazi_jewish==0 & allele_count_east_asian!=0 & allele_count_european_finnish==0 & allele_count_european_non_finnish==0 , allele_count_south_asian==0)}

rad51d_2_south_asian_unique <- function() {rad51d_2 %>% 
   filter(allele_count_african_african_american==0 & allele_count_other==0 & allele_count_latino_admixed_american==0 & allele_count_ashkenazi_jewish==0 & allele_count_east_asian==0 & allele_count_european_finnish==0 & allele_count_european_non_finnish==0 , allele_count_south_asian!=0)}
...
African American
```{r}
rad51d_2_african_table <- rad51d_2_african_unique() %>% 
...
Latino
```{r}
rad51d_2_latino_table <- rad51d_2_latino_unique() %>% 
    group_by(vep_annotation) %>% 
   mutate(clin_var_clinical_significance= case_when(clin_var_clinical_significance %in% Pathogenic ~ "Pathogenic", clin_var_clinical_significance %in% Uncertain ~ "Uncertain", clin_var_clinical_significance %in% Benign ~ "Benign", clin_var_clinical_significance=="not provided" ~ NA, TRUE~clin_var_clinical_significance)) %>% mutate(vep_annotation=case_when(vep_annotation %in% Splice ~ "splice_site_variant", vep_annotation %in% Inframe ~ "inframe_variant", vep_annotation%in%Stop ~ "stop_variant", TRUE ~ vep_annotation)) %>% 
    count(clin_var_clinical_significance) %>% mutate(Total= sum(n)) %>% 
  mutate(Percent_in_ClinVar=round(prop.table(n),4)*100) %>% 
...
European
```{r}
rad51d_2_european_table <- rad51d_2_european_unique() %>% 
    group_by(vep_annotation) %>% 
   mutate(clin_var_clinical_significance= case_when(clin_var_clinical_significance %in% Pathogenic ~ "Pathogenic", clin_var_clinical_significance %in% Uncertain ~ "Uncertain", clin_var_clinical_significance %in% Benign ~ "Benign", clin_var_clinical_significance=="not provided" ~ NA, TRUE~clin_var_clinical_significance)) %>% mutate(vep_annotation=case_when(vep_annotation %in% Splice ~ "splice_site_variant", vep_annotation %in% Inframe ~ "inframe_variant", vep_annotation%in%Stop ~ "stop_variant", TRUE ~ vep_annotation)) %>% 
    count(clin_var_clinical_significance) %>% mutate(Total= sum(n)) %>% 
  mutate(Percent_in_ClinVar=round(prop.table(n),4)*100) %>% 
...
East asian
```{r}
rad51d_2_east_asian_table <- rad51d_2_east_asian_unique() %>% 
  group_by(vep_annotation) %>% 
  mutate(clin_var_clinical_significance= case_when(clin_var_clinical_significance %in% Pathogenic ~ "Pathogenic", clin_var_clinical_significance %in% Uncertain ~ "Uncertain", clin_var_clinical_significance %in% Benign ~ "Benign", clin_var_clinical_significance=="not provided" ~ NA, TRUE~clin_var_clinical_significance)) %>% mutate(vep_annotation=case_when(vep_annotation %in% Splice ~ "splice_site_variant", vep_annotation %in% Inframe ~ "inframe_variant", vep_annotation%in%Stop ~ "stop_variant", TRUE ~ vep_annotation)) %>% 
  count(clin_var_clinical_significance) %>% mutate(Total= sum(n)) %>% 
  mutate(Percent_in_ClinVar=round(prop.table(n),4)*100) %>% 
...
South asian
```{r}
rad51d_2_south_asian_table <- rad51d_2_south_asian_unique() %>% 
  group_by(vep_annotation) %>% 
   mutate(clin_var_clinical_significance= case_when(clin_var_clinical_significance %in% Pathogenic ~ "Pathogenic", clin_var_clinical_significance %in% Uncertain ~ "Uncertain", clin_var_clinical_significance %in% Benign ~ "Benign", clin_var_clinical_significance=="not provided" ~ NA, TRUE~clin_var_clinical_significance)) %>% mutate(vep_annotation=case_when(vep_annotation %in% Splice ~ "splice_site_variant", vep_annotation %in% Inframe ~ "inframe_variant", vep_annotation%in%Stop ~ "stop_variant", TRUE ~ vep_annotation)) %>% 
  count(clin_var_clinical_significance) %>% mutate(Total= sum(n)) %>% 
  mutate(Percent_in_ClinVar=round(prop.table(n),4)*100) %>% 
...
rad51d_2 comparison of variants between population
```{r}
rad51d_2 %>% filter(allele_count_african_african_american!=0) %>% count(chromosome)->Total_rad51d_2_african
rad51d_2_african_unique() %>% count(chromosome) -> rad51d_2_african_uni
inner_join(Total_rad51d_2_african,rad51d_2_african_uni, by="chromosome") %>% mutate(n.z=n.x-n.y) %>% rename(Population=chromosome) -> African
African["Population"] <-  "African"

rad51d_2 %>% filter(allele_count_latino_admixed_american!=0) %>% count(chromosome)->Total_rad51d_2_latino
rad51d_2_latino_unique() %>% count(chromosome) -> rad51d_2_latino_uni
inner_join(Total_rad51d_2_latino,rad51d_2_latino_uni, by="chromosome") %>% mutate(n.z=n.x-n.y) %>% rename(Population=chromosome) -> Latino
Latino["Population"] <-  "Latino"

rad51d_2 %>% filter(allele_count_european_non_finnish!=0) %>% count(chromosome)->Total_rad51d_2_europe
rad51d_2_european_unique() %>% count(chromosome)->rad51d_2_europe_uni
inner_join(Total_rad51d_2_europe,rad51d_2_europe_uni, by="chromosome") %>% mutate(n.z=n.x-n.y) %>% rename(Population=chromosome) -> Europe
Europe["Population"] <-  "Europe"

rad51d_2 %>% filter(allele_count_east_asian!=0) %>% count(chromosome)->Total_rad51d_2_east_asia
rad51d_2_east_asian_unique() %>% count(chromosome)->rad51d_2_east_asia_uni
inner_join(Total_rad51d_2_east_asia,rad51d_2_east_asia_uni, by="chromosome") %>% mutate(n.z=n.x-n.y) %>% rename(Population=chromosome) -> East_asia
East_asia["Population"] <-  "East_asia"

rad51d_2 %>% filter(allele_count_south_asian!=0) %>% count(chromosome) -> Total_rad51d_2_south_asia
rad51d_2_south_asian_unique() %>% count(chromosome)->rad51d_2_south_asia_uni
inner_join(Total_rad51d_2_south_asia,rad51d_2_south_asia_uni, by="chromosome") %>% mutate(n.z=n.x-n.y) %>% rename(Population=chromosome) -> South_asia
South_asia["Population"] <-  "South_asia"

full_join(African, Latino) -> rad51d_2TUS
full_join(rad51d_2TUS,Europe) -> rad51d_2TUS
full_join(rad51d_2TUS,East_asia) -> rad51d_2TUS
full_join(rad51d_2TUS, South_asia) -> rad51d_2TUS
rad51d_2TUS %>% rename(Total=n.x, Unique=n.y, Shared=n.z) %>% select(-Total) %>% pivot_longer(cols = c(Unique, Shared), names_to = "Characteristics", values_to = "Values") %>% 
...
for the rad51d_2 gene", y="Number of variants")

ggsave(here("output/rad51d_2_pop.png"), width = 8, height = 6, dpi = 300)
```