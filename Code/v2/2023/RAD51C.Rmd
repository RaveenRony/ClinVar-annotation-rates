gnomAD 2023 - RAD51C - Population specific variant table & Comparison of unique and shared variants within population
...
Loading data
```{r}
rad51c <- clean_names(read_csv(here("data/rad51c.csv")))%>% mutate(gene="RAD51C",.before="chromosome")

rad51c_african_unique <- function() {rad51c %>% 
...
rad51c_latino_unique <- function() {rad51c %>% 
   filter(allele_count_african_african_american==0 & allele_count_other==0 & allele_count_latino_admixed_american!=0 & allele_count_ashkenazi_jewish==0 & allele_count_east_asian==0 & allele_count_european_finnish==0 & allele_count_european_non_finnish==0 , allele_count_south_asian==0)}

rad51c_european_unique <- function() {rad51c %>% 
   filter(allele_count_african_african_american==0 & allele_count_other==0 & allele_count_latino_admixed_american==0 & allele_count_ashkenazi_jewish==0 & allele_count_east_asian==0 & allele_count_european_finnish==0 & allele_count_european_non_finnish!=0 , allele_count_south_asian==0)}

rad51c_east_asian_unique <- function() {rad51c %>% 
   filter(allele_count_african_african_american==0 & allele_count_other==0 & allele_count_latino_admixed_american==0 & allele_count_ashkenazi_jewish==0 & allele_count_east_asian!=0 & allele_count_european_finnish==0 & allele_count_european_non_finnish==0 , allele_count_south_asian==0)}

rad51c_south_asian_unique <- function() {rad51c %>% 
   filter(allele_count_african_african_american==0 & allele_count_other==0 & allele_count_latino_admixed_american==0 & allele_count_ashkenazi_jewish==0 & allele_count_east_asian==0 & allele_count_european_finnish==0 & allele_count_european_non_finnish==0 , allele_count_south_asian!=0)}
...
African American
```{r}
rad51c_african_table <- rad51c_african_unique() %>% 
...
Latino
```{r}
rad51c_latino_table <- rad51c_latino_unique() %>% 
  group_by(vep_annotation) %>% 
  mutate(clin_var_clinical_significance= case_when(clin_var_clinical_significance %in% Pathogenic ~ "Pathogenic", clin_var_clinical_significance %in% Uncertain ~ "Uncertain", clin_var_clinical_significance %in% Benign ~ "Benign", clin_var_clinical_significance=="not provided" ~ NA, TRUE~clin_var_clinical_significance)) %>% mutate(vep_annotation=case_when(vep_annotation %in% Splice ~ "splice_site_variant", vep_annotation %in% Inframe ~ "inframe_variant", vep_annotation%in%Stop ~ "stop_variant", TRUE ~ vep_annotation)) %>% 
  count(clin_var_clinical_significance) %>% mutate(Total= sum(n)) %>% 
  mutate(Percent_In_ClinVar=round(prop.table(n),4)*100) %>% 
...
European
```{r}
rad51c_european_table <- rad51c_european_unique() %>% 
  group_by(vep_annotation) %>% 
  mutate(clin_var_clinical_significance= case_when(clin_var_clinical_significance %in% Pathogenic ~ "Pathogenic", clin_var_clinical_significance %in% Uncertain ~ "Uncertain", clin_var_clinical_significance %in% Benign ~ "Benign", clin_var_clinical_significance=="not provided" ~ NA, TRUE~clin_var_clinical_significance)) %>% mutate(vep_annotation=case_when(vep_annotation %in% Splice ~ "splice_site_variant", vep_annotation %in% Inframe ~ "inframe_variant", vep_annotation%in%Stop ~ "stop_variant", TRUE ~ vep_annotation)) %>% 
  count(clin_var_clinical_significance) %>% mutate(Total= sum(n)) %>% 
  mutate(Percent_In_ClinVar=round(prop.table(n),4)*100) %>% 
...
East asian
```{r}
rad51c_east_asian_table <- rad51c_east_asian_unique() %>% 
  group_by(vep_annotation) %>% 
  mutate(clin_var_clinical_significance= case_when(clin_var_clinical_significance %in% Pathogenic ~ "Pathogenic", clin_var_clinical_significance %in% Uncertain ~ "Uncertain", clin_var_clinical_significance %in% Benign ~ "Benign", clin_var_clinical_significance=="not provided" ~ NA, TRUE~clin_var_clinical_significance)) %>% mutate(vep_annotation=case_when(vep_annotation %in% Splice ~ "splice_site_variant", vep_annotation %in% Inframe ~ "inframe_variant", vep_annotation%in%Stop ~ "stop_variant", TRUE ~ vep_annotation)) %>% 
  count(clin_var_clinical_significance) %>% mutate(Total= sum(n)) %>% 
  mutate(Percent_In_ClinVar=round(prop.table(n),4)*100) %>% 
...
South asian
```{r}
rad51c_south_asian_table <- rad51c_south_asian_unique() %>% 
  group_by(vep_annotation) %>% 
  mutate(clin_var_clinical_significance= case_when(clin_var_clinical_significance %in% Pathogenic ~ "Pathogenic", clin_var_clinical_significance %in% Uncertain ~ "Uncertain", clin_var_clinical_significance %in% Benign ~ "Benign", clin_var_clinical_significance=="not provided" ~ NA, TRUE~clin_var_clinical_significance)) %>% mutate(vep_annotation=case_when(vep_annotation %in% Splice ~ "splice_site_variant", vep_annotation %in% Inframe ~ "inframe_variant", vep_annotation%in%Stop ~ "stop_variant", TRUE ~ vep_annotation)) %>% 
  count(clin_var_clinical_significance) %>% mutate(Total= sum(n)) %>% 
  mutate(Percent_In_ClinVar=round(prop.table(n),4)*100) %>% 
...
RAD51C comparison of variants between population
```{r}
rad51c %>% filter(allele_count_african_african_american!=0) %>% count(chromosome)->Total_rad51c_african
rad51c_african_unique() %>% count(chromosome) -> rad51c_african_uni
inner_join(Total_rad51c_african,rad51c_african_uni, by="chromosome") %>% mutate(n.z=n.x-n.y) %>% rename(Population=chromosome) -> African
African["Population"] <-  "African"

rad51c %>% filter(allele_count_latino_admixed_american!=0) %>% count(chromosome)->Total_rad51c_latino
rad51c_latino_unique() %>% count(chromosome) -> rad51c_latino_uni
inner_join(Total_rad51c_latino,rad51c_latino_uni, by="chromosome") %>% mutate(n.z=n.x-n.y) %>% rename(Population=chromosome) -> Latino
Latino["Population"] <-  "Latino"

rad51c %>% filter(allele_count_european_non_finnish!=0) %>% count(chromosome)->Total_rad51c_europe
rad51c_european_unique() %>% count(chromosome)->rad51c_europe_uni
inner_join(Total_rad51c_europe,rad51c_europe_uni, by="chromosome") %>% mutate(n.z=n.x-n.y) %>% rename(Population=chromosome) -> Europe
Europe["Population"] <-  "Europe"

rad51c %>% filter(allele_count_east_asian!=0) %>% count(chromosome)->Total_rad51c_east_asia
rad51c_east_asian_unique() %>% count(chromosome)->rad51c_east_asia_uni
inner_join(Total_rad51c_east_asia,rad51c_east_asia_uni, by="chromosome") %>% mutate(n.z=n.x-n.y) %>% rename(Population=chromosome) -> East_asia
East_asia["Population"] <-  "East_asia"

rad51c %>% filter(allele_count_south_asian!=0) %>% count(chromosome) -> Total_rad51c_south_asia
rad51c_south_asian_unique() %>% count(chromosome)->rad51c_south_asia_uni
inner_join(Total_rad51c_south_asia,rad51c_south_asia_uni, by="chromosome") %>% mutate(n.z=n.x-n.y) %>% rename(Population=chromosome) -> South_asia
South_asia["Population"] <-  "South_asia"

full_join(African, Latino) -> rad51cTUS
full_join(rad51cTUS,Europe) -> rad51cTUS
full_join(rad51cTUS,East_asia) -> rad51cTUS
full_join(rad51cTUS, South_asia) -> rad51cTUS
rad51cTUS %>% rename(Total=n.x, Unique=n.y, Shared=n.z) %>% select(-Total) %>% pivot_longer(cols = c(Unique, Shared), names_to = "Characteristics", values_to = "Values") %>% 
...
for the RAD51C gene", y="Number of variants")

ggsave(here("output/rad51c_pop.png"), width = 8, height = 6, dpi = 300)
```
